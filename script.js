...
for (i = 1; i < 8; i++) {
    document.getElementById("itens").innerHTML += `
      ...
      <td><Input Type="number" class="valores" id="totalIten${i}" onchange='proximoInput(iten${i + 1})' disabled></td>
    </tr>    
      `;
  }
...
if(x === els.length - 1){
    document.getElementById(els[x].id).setAttribute("onchange",`proximoInput('Itens1')` )

}else{
    console.log(els[x].id, x)
    let d= x + 1
  document.getElementById(els[x].id).setAttribute("onchange",`proximoInput(${els[d].id})` )
}
}
